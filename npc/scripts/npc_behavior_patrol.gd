...
func _ready() -> void:
	gather_patrol_lacations()
	if Engine.is_editor_hint():
		child_entered_tree.connect( gather_patrol_lacations )
		child_order_changed.connect( gather_patrol_lacations )
		return
	super()
...
func gather_patrol_lacations( _n : Node = null ) -> void:
	patrol_locations = []
	...
	if Engine.is_editor_hint():
		if patrol_locations.size() > 0:
			for i in patrol_locations.size():
				...
				if not _p.transform_changed.is_connected( gather_patrol_lacations ):
					_p.transform_changed.connect( gather_patrol_lacations )
